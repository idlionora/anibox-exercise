export const bannerCarouselItem = ({ id, title, description, banner, year, genre, format }) => {
...
export const detailCarousel = (streamingEpisodes) => {
	return `
      <h2 class="w-full max-w-[74rem] text-lg ">Stream Episodes</h2>
        <div class="relative mt-3 w-full max-w-[74rem] flex">
          <button id="ep-carousel-prev" class="flex flex-col justify-center cursor-pointer shrink-0 grow-0">
            <svg aria-hidden="true" class="w-10 h-10 pr-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            <span class="sr-only">Previous</span>
          </button>
          <div id="episodes-container" class="flex flex-nowrap overflow-x-scroll scroll-smooth hide-scroll-bar gap-x-5 snap-x snap-mandatory">
            ${streamingEpisodes
				.map((episode) =>
					episodeCard({
						title: episode.title,
          ...
          <button id="ep-carousel-next" class="flex flex-col justify-center cursor-pointer shrink-0 grow-0">
            <svg aria-hidden="true" class="w-10 h-10 pl-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
          </svg>
            <span class="sr-only">Previous</span>
...
const episodeCard = ({ title, url, thumbnail, site }) => {
	if (title.length > 50) {
		title = title.substring(0, 50) + '...';
	}
	return `
    <a href="${url}" class="snap-start cursor-pointer w-56">
      <div class="w-[10.3rem] h-56 overflow-hidden rounded-lg shadow-md bg-white hover:shadow-xl transition-shadow duration-300 ease-in-out">
        <img class="block object-cover h-56 w-56" src="${thumbnail}" alt="${title}">
      </div>
      <div class="mt-4">
        <p class="text-white">${title}</p>
        <p class="text-sm text-gray-400 mt-1">${site}</p>
      </div>
    </a>
  `;
};
